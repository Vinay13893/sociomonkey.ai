      ...
      '<div class="card" style="max-width:420px;width:100%;text-align:center;padding:40px 32px;">' +
        ...
        '<div style="display:flex;flex-direction:column;gap:10px;">' +
          (user && user.tenant_slug
            ...
            : '') +
          '<button onclick="var _sl=(typeof authIsTenantUser===\'function\'&&authIsTenantUser()&&user&&user.tenant_slug)?user.tenant_slug:null;authClearSession();clearTenantContext();history.replaceState({},' + "''" + ',_sl?\'/\'+_sl+\'/login\':\'/login\');dispatch()" class="button" style="font-size:14px;background:#f1f5f9;color:#64748b;border:1px solid #e2e8f0;">Sign Out</button>' +
        '</div>' +
      '</div>' +
    '</div>'
}
