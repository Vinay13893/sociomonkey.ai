...
var _dashboardStatsPromises  = {}
var _dashboardStatsMemoryCache = {}
var _DASHBOARD_STATS_CACHE_TTL_MS = 15000

function _dashboardStatsCacheKey(params) {
  var qs = params && typeof params.toString === 'function' ? params.toString() : ''
...
async function renderDashboard() {  // ── Dedup: if a render is already in flight, skip rather than cancel it ──
  if (_dashboardRenderInFlight) {
    if (window.DEBUG_PERF) console.warn('[renderDashboard] superseding an in-flight render')
  }
  ...
  function safeMutate(elOrId, html) {
    ...
    if (window.DEBUG_PERF) console.log('[DOM WRITE]', _id, window._ACTIVE_ROUTE, window._ACTIVE_RENDER_ID)
  ...
  // ── Projects: populate dropdown in background — don't block event wiring ──
    ...
    if (!data) {
      ...
      const cacheKey = _dashboardStatsCacheKey(params)
      const cached = _dashboardStatsMemoryCache[cacheKey]
      if (cached && Date.now() - cached.ts < _DASHBOARD_STATS_CACHE_TTL_MS) {
        data = cached.data
      }
    }
    if (!data) {
      ...
      try {
        ...
        if (!_dashboardStatsPromises[cacheKey]) {
          _dashboardStatsPromises[cacheKey] = _dashboardFetchJsonWithTimeout(
            url,
            ...
            delete _dashboardStatsPromises[cacheKey]
          })
        }
        data = await _dashboardStatsPromises[cacheKey]
        _dashboardStatsMemoryCache[cacheKey] = { ts: Date.now(), data: data }
...
// Legacy notification toast poller intentionally disabled.
// In-app notifications are surfaced via the bell component only
// (`src/shared/ui/notifications.js`) to avoid disruptive toast replay.
