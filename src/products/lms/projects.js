...
async function renderProjects() {
  ...
  content.innerHTML = `
    <div class="card">
      <div class="header">
        <h2>Projects</h2>
        ${user && user.role === 'superadmin' ? '<button class="sm-btn sm-btn-primary" id="newProjectBtn">+ Add Project</button>' : ''}
      </div>
  ...
  if (newProjBtn) newProjBtn.addEventListener('click', function () { openProjectForm() })
...
async function openProjectForm(project = null) {
  const isEdit = !!(project && typeof project === 'object' && typeof project.id === 'number')
  const content = document.getElementById('content')
  if (!content) return
  ...
  content.innerHTML = `
    <div class="card" style="max-width:600px;">
      <h2 class="sm-section-heading" style="margin-bottom:20px;">${isEdit ? 'Edit Project' : 'Add Project'}</h2>
      <form id="projectForm">
        ...
        <div style="display:flex;gap:10px;margin-top:20px;">
          <button type="submit" class="button" id="projectSubmitBtn">${isEdit ? 'Update Project' : 'Add Project'}</button>
  ...
  document.getElementById('projectForm').addEventListener('submit', async e => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showToast(payload.error || (isEdit ? 'Unable to update project' : 'Unable to add project'), 'error')
        return
      }
      showToast(isEdit ? 'Project updated' : 'Project added', 'success')
      renderProjects()
    } catch (err) {
      showToast((err && err.message) || 'Request failed', 'error')
    } finally {
