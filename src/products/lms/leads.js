...
var _leadSourceNames = []
var _leadSourceNamesTs = 0
var _leadsAutoRefreshTimer = null
var _LEADS_AUTO_FETCH_MS = 15 * 60 * 1000
var _leadsById = {}

var BASE_LEAD_SOURCES = ['Website','Referral','Walk-in','Meta','Google','Email Campaign','Direct','Other','G1','G2','G3','TP']

function _buildLeadsListQueryParams(options) {
  var opts = options || {}
  ...
  var teamMemberFilter = document.getElementById('filterTeamMember')?.value || ''
  var salesManagerFilter = document.getElementById('filterSalesManager')?.value || ''
  var dateFrom = document.getElementById('filterDateFrom')?.value || ''
  var dateTo = document.getElementById('filterDateTo')?.value || ''
  var sortOrder = document.getElementById('quickSortOrder')?.value || 'new_old'

  if (statusFilter) params.set('status', statusFilter)
  if (projectFilter) params.set('project_id', projectFilter)
  if (sourceFilter) params.set('source', sourceFilter)
  if (teamMemberFilter) params.set('assigned_to', teamMemberFilter)
  if (salesManagerFilter) params.set('sales_manager_id', salesManagerFilter)
  if (dateFrom) params.set('date_from', dateFrom)
  if (dateTo) params.set('date_to', dateTo)
  if (_leadsSearchQuery) params.set('q', _leadsSearchQuery)
...
function _rememberLeadRows(rows) {
  ;(rows || []).forEach(function (row) {
    if (row && row.id) _leadsById[row.id] = Object.assign({}, _leadsById[row.id] || {}, row)
  })
}

function _mergeLeadRows(rows) {
  _rememberLeadRows(rows)
  ;(rows || []).forEach(function (row) {
    if (!row || !row.id) return
    var idx = (leads || []).findIndex(function (lead) { return Number(lead.id) === Number(row.id) })
    if (idx >= 0) leads[idx] = Object.assign({}, leads[idx], row)
    if (typeof _leadsPageCache === 'object' && _leadsPageCache) {
      Object.keys(_leadsPageCache).forEach(function (key) {
        var cached = _leadsPageCache[key]
        if (!cached || !Array.isArray(cached.leads)) return
        var cacheIdx = cached.leads.findIndex(function (lead) { return Number(lead.id) === Number(row.id) })
        if (cacheIdx >= 0) cached.leads[cacheIdx] = Object.assign({}, cached.leads[cacheIdx], row)
      })
    }
  })
}

async function _refreshLeadRows(ids) {
  var uniqueIds = Array.from(new Set((ids || []).map(Number).filter(Boolean))).slice(0, 100)
  if (!uniqueIds.length) return []
  var data = await loadLeads({ ids: uniqueIds, force: true })
  var rows = data && data.leads ? data.leads : []
  _mergeLeadRows(rows)
  return rows
}

function _removeLeadRows(ids) {
  var removeSet = new Set((ids || []).map(Number))
  leads = (leads || []).filter(function (row) { return !removeSet.has(Number(row.id)) })
  if (typeof _leadsPageCache === 'object' && _leadsPageCache) {
    Object.keys(_leadsPageCache).forEach(function (key) {
      var cached = _leadsPageCache[key]
      if (cached && Array.isArray(cached.leads)) cached.leads = cached.leads.filter(function (row) { return !removeSet.has(Number(row.id)) })
    })
  }
  removeSet.forEach(function (id) {
    delete _leadsById[id]
    selectedLeads.delete(id)
  })
  if (typeof invalidateLeadsCache === 'function') invalidateLeadsCache()
}

async function _refreshLeadDeltas() {
  var since = _leadsLastServerTime || ''
  if (!since) {
    await loadLeads({ force: true })
    _rememberLeadRows(leads)
    return
  }
  var data = await loadLeads({ updated_since: since, page_size: 100, force: true })
  var rows = data && data.leads ? data.leads : []
  if (!rows.length) return
  _mergeLeadRows(rows)
  if (leadsPage === 1) await loadLeads({ force: true })
}

function _leadSourceOptionNames() {
  var seen = {}
  return BASE_LEAD_SOURCES.concat(_leadSourceNames || []).filter(function (name) {
...
function _leadSourceOptionsHtml(selected, placeholder) {
  var selectedValue = String(selected || '')
  return `<option value="">${escape(placeholder || 'All Sources')}</option>` +
    _leadSourceOptionNames().map(function (name) {
      return `<option value="${escape(name)}" ${selectedValue === name ? 'selected' : ''}>${escape(name)}</option>`
    }).join('')
}

async function loadLeadSourceNames(force) {
  if (!force && _leadSourceNamesTs && Date.now() - _leadSourceNamesTs < 60000) return _leadSourceNames
  try {
    var res = await authFetch('/api/lead-sources?include_inactive=true')
    var data = await res.json()
    var rows = Array.isArray(data.sources) ? data.sources : []
    _leadSourceNames = rows
      .filter(function (row) { return row && row.is_active !== false && row.name })
      .map(function (row) { return String(row.name || '').trim() })
      .filter(Boolean)
    _leadSourceNamesTs = Date.now()
  } catch (_) {
    _leadSourceNames = _leadSourceNames || []
  }
  return _leadSourceNames
}

function populateLeadSourceSelect(selectId, selected, placeholder) {
  var el = document.getElementById(selectId)
  if (el) el.innerHTML = _leadSourceOptionsHtml(selected, placeholder)
}

function _startLeadsAutoRefresh() {
  if (_leadsAutoRefreshTimer) clearInterval(_leadsAutoRefreshTimer)
  async function fetchAndRefresh() {
    if (document.visibilityState !== 'visible') return
    if (String(activeTab || '') !== 'leads') return
    if (!document.querySelector('.leads-workbench')) return
    await _maybeRealtimeMetaSync()
    await _refreshLeadDeltas()
    await filterAndRenderLeads(false)
  }
  setTimeout(fetchAndRefresh, 0)
  _leadsAutoRefreshTimer = setInterval(fetchAndRefresh, _LEADS_AUTO_FETCH_MS)
}
...
async function renderLeads() {
      ...
      <div class="leads-command-shell">
        ...
        <div id="leadsAdvancedFilters" class="leads-advanced-filters" style="display:${_leadsAdvancedOpen ? 'grid' : 'none'};">
          ...
          <div class="dash-filter-group">
            ...
            <select id="filterSource" class="dash-filter-ctl">${_leadSourceOptionsHtml('', 'All Sources')}</select>
          </div>
        </div>
      </div>
  ...
  await Promise.all([loadProjects(), loadUsers(), loadLeads(), loadLeadSourceNames()])
  populateLeadSourceSelect('filterSource', '', 'All Sources')
...
async function saveLeadInlineNote(leadId) {
  ...
  try {
    ...
    _mergeLeadRows([{ id: leadId, latest_note: noteText.slice(0, 120) }])
    await filterAndRenderLeads(false)
...
function _leadAgeOldLabel(createdAt) {
  ...
  var timeText = dt.toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: 'numeric', minute: '2-digit' })
  var todayYmd = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Kolkata', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date())
  var leadYmd = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Kolkata', year: 'numeric', month: '2-digit', day: '2-digit' }).format(dt)
  if (leadYmd === todayYmd) return 'Today ' + timeText
...
async function filterAndRenderLeads(resetPage = true) {
  ...
  const totalLeads = Number((_leadsPagination && _leadsPagination.total) || filtered.length || 0)
  const totalPages = Math.max(1, Number((_leadsPagination && _leadsPagination.total_pages) || Math.ceil(totalLeads / leadsPageSize) || 1))
  if (leadsPage > totalPages) leadsPage = totalPages
  if (leadsPage < 1) leadsPage = 1
  const pageStart = totalLeads === 0 ? 0 : (leadsPage - 1) * leadsPageSize
  ...
  statusSelects.forEach(function (sel) {
    sel.addEventListener('change', async function () {
      ...
      sel.disabled = true
      try {
        var data = await _apiRequest(`/pipeline/leads/${leadId}/move`, {
          method: 'POST',
          headers: { ..._apiAuthHeaders(), ..._apiJsonHeaders() },
        ...
        if (data && data.lead) _mergeLeadRows([data.lead])
        else _mergeLeadRows([{ id: leadId, status: nextStatus }])
        await filterAndRenderLeads(false)
...
async function bulkUpdateStatus() {
  ...
  try {
    ...
    const changedIds = [...selectedLeads]
    await _refreshLeadRows(changedIds)
    selectedLeads.clear()
    await filterAndRenderLeads(false)
    const bar = document.getElementById('bulkBar')
    if (bar) {
...
async function bulkDeleteLeads() {
  ...
  try {
    ...
    const deletedIds = [...selectedLeads]
    selectedLeads.clear()
    _removeLeadRows(deletedIds)
    filterAndRenderLeads(false)
    const bar = document.getElementById('bulkBar')
    if (bar) {
      bar.style.display = 'flex'
...
async function bulkExportSelectedLeads() {
  ...
  var missingIds = ids.filter(function (id) { return !_leadsById[id] })
  if (missingIds.length) await _refreshLeadRows(missingIds)
  var selected = ids.map(function (id) { return _leadsById[id] }).filter(Boolean)
  if (!selected.length) {
...
async function openLeadAssignModal(leadId) {
  ...
  if (btn) btn.addEventListener('click', async function () {
    ...
    try {
      var data = await _apiRequest(`/leads/${id}/assign`, {
        method: 'POST',
        headers: { ..._apiAuthHeaders(), ..._apiJsonHeaders() },
      ...
      overlay.remove()
      if (data && data.lead) _mergeLeadRows([data.lead])
      else await _refreshLeadRows([id])
      await filterAndRenderLeads(false)
...
async function viewLeadDetails(leadId) {
              ...
              <div>
                ...
                <form id="updateSourceForm" style="display:flex;gap:8px;">
                  <select id="newSource" class="select" style="flex:1;font-size:13px;">
                    ...
                    ${_leadSourceOptionsHtml(L.source, '— Select Source —')}
                  </select>
                  <button type="submit" class="button" style="font-size:12px;padding:7px 14px;white-space:nowrap;">Update</button>
                </form>
              </div>
  ...
  if (updateStatusForm) {
    updateStatusForm.addEventListener('submit', async e => {
      ...
      try {
        var data = await _apiRequest(`/leads/${leadId}`, {
          method: 'PUT',
          headers: { ..._apiAuthHeaders(), ..._apiJsonHeaders() },
  ...
  if (assignForm) {
    ...
    assignForm.addEventListener('submit', async e => {
      ...
      try {
        var data = await _apiRequest(`/leads/${leadId}/assign`, {
          method: 'POST',
          headers: { ..._apiAuthHeaders(), ..._apiJsonHeaders() },
...
function openLeadEditForm(lead) {
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
          ...
          <div>
            ...
            <select class="select" id="editSource" style="font-size:13px;">
              ...
              ${_leadSourceOptionsHtml(lead.source, '— Source —')}
            </select>
          </div>
        </div>
        <div>
  ...
  document.getElementById('editLeadForm').addEventListener('submit', async e => {
    ...
    try {
      var data = await _apiRequest(`/leads/${lead.id}`, {
        method: 'PUT',
        headers: { ..._apiAuthHeaders(), ..._apiJsonHeaders() },
        body: JSON.stringify({
...
async function openLeadForm() {
  const content = document.getElementById('content')
  await Promise.all([loadProjects(), loadLeadSourceNames()])
  ...
  content.innerHTML = `
    <div class="card" style="max-width:600px;">
      ...
      <form id="leadForm">
        ...
        <select class="select" id="leadSource">${_leadSourceOptionsHtml('', 'Select Source')}</select>
