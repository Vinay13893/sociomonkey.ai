...
var _abRequestCache = {}
var _abRequestPromises = {}
var _AB_CACHE_TTL_MS = 15000
var _abSectionPages = {
  today_callbacks: 1,
  overdue_callbacks: 1,
  new_leads_today: 1,
...
function _abCacheKey(queryString) {
  var tenant = String(platformTenantSlug || (user && user.tenant_slug) || (user && user.tenant_id) || 'tenant')
  var role = String((user && user.role) || 'role')
  var uid = String((user && user.id) || 'anon')
  return tenant + '|' + role + '|' + uid + '|' + String(queryString || '')
}
...
async function renderActionBoard(dateFrom, dateTo, rangeKey) {
  ...
  const fmtHeader = function (d) { return new Date(d).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata', weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) }
  ...
  function _boardRangeTitle() {
    if (_abRange === 'today') return "Today's Action Board"
    if (_abRange === 'this_week') return 'This Week Action Board'
    if (_abRange === 'this_month') return 'This Month Action Board'
    if (_abRange === 'last_30_days') return 'Last 30 Days Action Board'
    if (_abRange === 'custom') {
      if (queryFrom && queryTo) return fmtHeader(queryFrom) + ' - ' + fmtHeader(queryTo) + ' Action Board'
      if (queryFrom) return fmtHeader(queryFrom) + ' - Today Action Board'
      if (queryTo) return 'Until ' + fmtHeader(queryTo) + ' Action Board'
      return 'Custom Action Board'
    }
    return 'All Time Action Board'
  }
  const boardTitle = _boardRangeTitle()
  ...
  if (typeof window.smUpgradeDateRangeFilters === 'function') {
    setTimeout(function () { window.smUpgradeDateRangeFilters() }, 0)
  }
  ...
  try {
    ...
    const cacheKey = _abCacheKey(qs)
    const cached = _abRequestCache[cacheKey]
    if (cached && Date.now() - cached.ts < _AB_CACHE_TTL_MS) {
      data = cached.data
    } else {
      if (!_abRequestPromises[cacheKey]) {
        _abRequestPromises[cacheKey] = _apiRequest(`/leads/action-board${qs}`, {
          headers: _apiAuthHeaders(),
          retries: 2,
          timeoutMs: 20000,
        }).finally(function () {
          delete _abRequestPromises[cacheKey]
        })
      }
      data = await _abRequestPromises[cacheKey]
      _abRequestCache[cacheKey] = { ts: Date.now(), data: data }
    }
  } catch (err) {
    if (!_guard()) return
  ...
  function _callbackRow(c) {
    ...
    const waCallbackBtn = (phone || alternatePhone)
      ...
      : ''
    const timeStr = dt.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })
...
function _abRenderWorkspace() {
  ...
  if (refreshBtn) refreshBtn.addEventListener('click', function () { _abInvalidateCache(); renderActionBoard(_abDateFrom, _abDateTo, _abRange) })
}
